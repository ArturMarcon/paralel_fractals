#include <mpi.h>
#include <omp.h>
#include <stdio.h>
#include <stdlib.h>
#include <math.h>

#define WIDTH 1920
#define HEIGHT 1080
#define MAX_ITER 2000

// MPI Message Tags
#define TAG_REQUEST     0   // worker -> master: "I want work"
#define TAG_WORK        1   // master -> worker: here's a frame_id to compute
#define TAG_DIE         2   // master -> worker: no more work, terminate
#define TAG_RESULT_BASE 3   // worker -> master: result tag = TAG_RESULT_BASE + frame_id

const double C_REAL = -0.7;
const double C_IMAG = 0.27015;

const int COLOR1_R = 10;   const int COLOR1_G = 15;   const int COLOR1_B = 45;
const int COLOR2_R = 0;    const int COLOR2_G = 225;  const int COLOR2_B = 235;
const int COLOR3_R = 140;  const int COLOR3_G = 25;   const int COLOR3_B = 230;

void compute_julia_frame(int frame_id, double center_real, double center_imag, unsigned char *buffer) {
    double zoom = pow(0.94, frame_id);
    double base_width = 3.0;
    double current_width = base_width * zoom;
    double current_height = current_width * ((double)HEIGHT / (double)WIDTH);

    // OPENMP WORKPOOL (Without Master, all threads participate computing lines)
    #pragma omp parallel for schedule(dynamic)
    for (int y = 0; y < HEIGHT; y++) {
        for (int x = 0; x < WIDTH; x++) {
            double z_real = center_real + (x - WIDTH / 2.0) * (current_width / WIDTH);
            double z_imag = center_imag + (y - HEIGHT / 2.0) * (current_height / HEIGHT);

            int iter = 0;
            double z_real_sq = z_real * z_real;
            double z_imag_sq = z_imag * z_imag;

            while (z_real_sq + z_imag_sq <= 4.0 && iter < MAX_ITER) {
                double temp = z_real_sq - z_imag_sq + C_REAL;
                z_imag = 2.0 * z_real * z_imag + C_IMAG;
                z_real = temp;
                z_real_sq = z_real * z_real;
                z_imag_sq = z_imag * z_imag;
                iter++;
            }

            size_t pixel_idx = (size_t)(y * WIDTH + x) * 3;

            if (iter == MAX_ITER) {
                buffer[pixel_idx]     = 0;
                buffer[pixel_idx + 1] = 0;
                buffer[pixel_idx + 2] = 0;
            } else {
                double mu = (double)iter / MAX_ITER;
                if (mu < 0.5) {
                    double t = mu * 2.0;
                    buffer[pixel_idx]     = (unsigned char)((1.0 - t) * COLOR1_R + t * COLOR2_R);
                    buffer[pixel_idx + 1] = (unsigned char)((1.0 - t) * COLOR1_G + t * COLOR2_G);
                    buffer[pixel_idx + 2] = (unsigned char)((1.0 - t) * COLOR1_B + t * COLOR2_B);
                } else {
                    double t = (mu - 0.5) * 2.0;
                    buffer[pixel_idx]     = (unsigned char)((1.0 - t) * COLOR2_R + t * COLOR3_R);
                    buffer[pixel_idx + 1] = (unsigned char)((1.0 - t) * COLOR2_G + t * COLOR3_G);
                    buffer[pixel_idx + 2] = (unsigned char)((1.0 - t) * COLOR2_B + t * COLOR3_B);
                }
            }
        }
    }
}

void save_ppm(int frame_id, unsigned char *buffer) {
    char filename[64];
    sprintf(filename, "frame_%04d.ppm", frame_id);
    FILE *fp = fopen(filename, "wb");
    if (fp) {
        fprintf(fp, "P6\n%d %d\n255\n", WIDTH, HEIGHT);
        fwrite(buffer, sizeof(unsigned char), (size_t)WIDTH * HEIGHT * 3, fp);
        fclose(fp);
    }
}

int main(int argc, char *argv[]) {
    int rank, size;
    int provided;
    MPI_Init_thread(&argc, &argv, MPI_THREAD_FUNNELED, &provided);
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    MPI_Comm_size(MPI_COMM_WORLD, &size);

    double target_real = -0.1;
    double target_imag = -0.1;
    int total_frames = 128; // Updated to 128 as requested
    int save_files = 0; // Disable file saving by default to avoid I/O bottlenecks in profiling

    if (argc >= 3) {
        target_real = atof(argv[1]);
        target_imag = atof(argv[2]);
    }
    if (argc >= 4) total_frames = atoi(argv[3]);
    if (argc >= 5) save_files = atoi(argv[4]);

    if (size < 2) {
        if (rank == 0) fprintf(stderr, "Error: Master-Worker requires >= 2 MPI processes.\n");
        MPI_Finalize();
        return 1;
    }

    size_t frame_buffer_size = (size_t)WIDTH * HEIGHT * 3 * sizeof(unsigned char);

    if (rank == 0) {
        // MASTER RANK
        MPI_Barrier(MPI_COMM_WORLD);
        double start_time = MPI_Wtime();
        double io_time = 0.0;

        int next_frame = 0;
        int frames_saved = 0;
        int active_workers = size - 1;

        unsigned char *recv_buffer = (unsigned char *)malloc(frame_buffer_size);
        while (frames_saved < total_frames || active_workers > 0) {
            MPI_Status status;
            MPI_Probe(MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &status);

            int worker_id = status.MPI_SOURCE;
            int tag = status.MPI_TAG;

            if (tag == TAG_REQUEST) {
                int dummy;
                MPI_Recv(&dummy, 1, MPI_INT, worker_id, TAG_REQUEST, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

                if (next_frame < total_frames) {
                    MPI_Send(&next_frame, 1, MPI_INT, worker_id, TAG_WORK, MPI_COMM_WORLD);
                    next_frame++;
                } else {
                    int term = -1;
                    MPI_Send(&term, 1, MPI_INT, worker_id, TAG_DIE, MPI_COMM_WORLD);
                    active_workers--;
                }
            } else {
                int completed_frame_id = tag - TAG_RESULT_BASE;
                MPI_Recv(recv_buffer, (int)frame_buffer_size, MPI_BYTE, worker_id, tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                if (save_files) {
                    double io0 = MPI_Wtime();
                    save_ppm(completed_frame_id, recv_buffer);
                    io_time += MPI_Wtime() - io0;
                }
                frames_saved++;
            }
        }
        free(recv_buffer);
        
        double end_time = MPI_Wtime();
        double total_time = end_time - start_time;

        double local_stats[2] = {0.0, 0.0};
        double *all_stats = (double *)malloc(2 * size * sizeof(double));
        MPI_Gather(local_stats, 2, MPI_DOUBLE, all_stats, 2, MPI_DOUBLE, 0, MPI_COMM_WORLD);

        double max_compute = 0.0, min_compute = 1e18, sum_compute = 0.0;
        int total_frames_check = 0;
        for (int worker = 1; worker < size; worker++) {
            int wframes = (int)all_stats[2 * worker];
            double wtime = all_stats[2 * worker + 1];
            total_frames_check += wframes;
            if (wtime > max_compute) max_compute = wtime;
            if (wtime < min_compute) min_compute = wtime;
            sum_compute += wtime;
        }
        double avg_compute = sum_compute / (size - 1);
        double imbalance = (max_compute - min_compute) / (max_compute > 0 ? max_compute : 1.0);

        // CSV OUTPUT: format -> CSV,kind,np,workers,omp_threads,total_frames,total_s,io_s,compute_max_s,compute_min_s,imbalance_pct
        int omp_threads = 0;
        #pragma omp parallel
        {
            #pragma omp single
            omp_threads = omp_get_num_threads();
        }
        
        // Since master doesn't run the worker's thread pool, omp_get_num_threads() here might just be 1.
        // We will pass omp_threads via env var in the script, or we can trust the worker. We'll just read from env.
        char* env_omp = getenv("OMP_NUM_THREADS");
        int threads_val = env_omp ? atoi(env_omp) : 1;

        printf("CSV,hybrid,%d,%d,%d,%d,%.6f,%.6f,%.6f,%.6f,%.4f\n",
               size, size - 1, threads_val, total_frames,
               total_time, io_time, max_compute, min_compute, imbalance * 100.0);
               
        free(all_stats);
    } else {
        // WORKER RANK
        unsigned char *buffer0 = (unsigned char *)malloc(frame_buffer_size);
        unsigned char *buffer1 = (unsigned char *)malloc(frame_buffer_size);
        int frames_processed = 0;
        double compute_time = 0.0;
        int request_payload = rank;

        MPI_Barrier(MPI_COMM_WORLD);

        int frame_curr;
        MPI_Status status;
        
        // 1. Send first request and wait for first frame synchronously
        MPI_Send(&request_payload, 1, MPI_INT, 0, TAG_REQUEST, MPI_COMM_WORLD);
        MPI_Recv(&frame_curr, 1, MPI_INT, 0, MPI_ANY_TAG, MPI_COMM_WORLD, &status);

        if (status.MPI_TAG != TAG_DIE) {
            MPI_Request req_send_result = MPI_REQUEST_NULL;
            unsigned char *buf_compute = buffer0;
            unsigned char *buf_send = buffer1;

            while (1) {
                int frame_next;
                MPI_Request req_recv_frame;
                
                // 2. Ask for the NEXT frame proactively
                MPI_Send(&request_payload, 1, MPI_INT, 0, TAG_REQUEST, MPI_COMM_WORLD);
                // 3. Start receiving NEXT frame asynchronously
                MPI_Irecv(&frame_next, 1, MPI_INT, 0, MPI_ANY_TAG, MPI_COMM_WORLD, &req_recv_frame);

                // 4. Compute CURRENT frame
                double c0 = MPI_Wtime();
                compute_julia_frame(frame_curr, target_real, target_imag, buf_compute);
                compute_time += MPI_Wtime() - c0;
                frames_processed++;

                // 5. Wait for previous Isend to finish before we send the newly computed buffer
                if (req_send_result != MPI_REQUEST_NULL) {
                    MPI_Wait(&req_send_result, MPI_STATUS_IGNORE);
                }

                // 6. Start sending the newly computed buffer
                MPI_Isend(buf_compute, (int)frame_buffer_size, MPI_BYTE, 0, TAG_RESULT_BASE + frame_curr, MPI_COMM_WORLD, &req_send_result);

                // 7. Wait for the NEXT frame to arrive
                MPI_Status status_recv;
                MPI_Wait(&req_recv_frame, &status_recv);

                // 8. If the NEXT frame is a DIE signal, we exit the loop
                if (status_recv.MPI_TAG == TAG_DIE) {
                    break;
                }
                
                // Set up for next iteration
                frame_curr = frame_next;
                
                // Swap buffers
                unsigned char *tmp = buf_compute;
                buf_compute = buf_send;
                buf_send = tmp;
            }

            // Clean up last send before exiting
            if (req_send_result != MPI_REQUEST_NULL) {
                MPI_Wait(&req_send_result, MPI_STATUS_IGNORE);
            }
        }

        double local_stats[2] = { (double)frames_processed, compute_time };
        MPI_Gather(local_stats, 2, MPI_DOUBLE, NULL, 0, MPI_DOUBLE, 0, MPI_COMM_WORLD);
        free(buffer0);
        free(buffer1);
    }

    MPI_Finalize();
    return 0;
}
